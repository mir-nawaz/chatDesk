...
module.exports = [
  function(session, results, next) {
    session.send('Lets play a small video how you can use to check with a different handset');
    const msg = new builder.Message(session).addAttachment(

      new builder.VideoCard(session)
        .title('Try a different Handset')
        .subtitle('')
        .text('Watch this amazing movie which will be repalced by an actual help manual but now enjoy without thinking so much about it')
